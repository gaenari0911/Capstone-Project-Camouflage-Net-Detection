...
class v8SegmentationLoss(v8DetectionLoss):
    ...
    def loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if fg_mask.sum():
            ...
            # ==========================================
            # [커스텀 추가] Boundary Loss 계산 및 합산
            # ==========================================
            if "boundary" in preds and "boundaries" in batch and "distance_maps" in batch:
                pred_boundary = preds["boundary"]  # (B, 1, H_p, W_p)
                B, _, H_p, W_p = pred_boundary.shape
                
                gt_boundary = batch["boundaries"].to(self.device, dtype=torch.float32) # (N_instances, H_gt, W_gt)
                distance_weight = batch["distance_maps"].to(self.device, dtype=torch.float32) # (N_instances, H_gt, W_gt)
                batch_idx = batch["batch_idx"].view(-1).long() # (N_instances,)

                # [추가된 로직] 객체 단위(36개)를 이미지 단위(16장)로 병합
                _, H_gt, W_gt = gt_boundary.shape
                gt_boundary_merged = torch.zeros((B, H_gt, W_gt), device=self.device, dtype=torch.float32)
                dist_weight_merged = torch.ones((B, H_gt, W_gt), device=self.device, dtype=torch.float32) # 기본 가중치 1.0

                for i in range(B):
                    idx = (batch_idx == i) # i번째 이미지에 속하는 객체들의 인덱스
                    if idx.sum() > 0:      # 해당 이미지에 객체가 1개 이상 존재할 경우
                        # 겹치는 경계선과 가중치를 합치기 위해 max() 사용
                        # 여러 객체가 겹쳐도 가장 뚜렷한 경계선(1)과 가장 높은 거리 가중치가 남게 됨
                        gt_boundary_merged[i] = gt_boundary[idx].max(dim=0)[0]
                        dist_weight_merged[i] = distance_weight[idx].max(dim=0)[0]

                # 텐서 크기(해상도) 맞추기: (B, H_gt, W_gt) -> (B, 1, H_p, W_p)
                gt_boundary_resized = F.interpolate(
                    gt_boundary_merged.unsqueeze(1), size=(H_p, W_p), mode="bilinear", align_corners=False
                )
                distance_weight_resized = F.interpolate(
                    dist_weight_merged.unsqueeze(1), size=(H_p, W_p), mode="bilinear", align_corners=False
                )

                # BCE 손실 계산 (픽셀 단위 계산을 위해 reduction='none')
                bce_loss_fn = nn.BCEWithLogitsLoss(reduction="none")
                loss_b_pixelwise = bce_loss_fn(pred_boundary, gt_boundary_resized)

                # Distance Map 가중치 적용 후 평균 계산
                loss_b_weighted = loss_b_pixelwise * distance_weight_resized
                loss_boundary = loss_b_weighted.mean()

                # 기존 Seg Loss(loss[1])에 Boundary Loss 합산
                lambda_boundary = 1.0  
                loss[1] += lambda_boundary * loss_boundary
            # ==========================================
        ...
        # WARNING: lines below prevent Multi-GPU DDP 'unused gradient' PyTorch errors, do not remove
            ...
            # ==========================================
            # [커스텀 추가] DDP 미사용 그래디언트 에러 방지
            # ==========================================
            if "boundary" in preds:
