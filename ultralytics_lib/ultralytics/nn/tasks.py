    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
        ...
        elif m in frozenset(
            {
                ...
                OBB,
                OBB26,
                DualHeadSegment,    # DualHeadSegment 추가
            }
        ):
            args.extend([reg_max, end2end, [ch[x] for x in f]])
            if m is Segment or m is YOLOESegment or m is Segment26 or m is YOLOESegment26 or m is DualHeadSegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26, DualHeadSegment}:
                m.legacy = legacy
        elif m is v10Detect:
...
def guess_model_task(model):
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment, DualHeadSegment)):
                return "segment"
