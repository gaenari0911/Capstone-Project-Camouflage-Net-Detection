...
class DualHeadSegment(Segment):
    """
    기존 YOLOv8-seg의 Mask Head 기능에 Boundary Head를 추가한 커스텀 모델
    """
    def __init__(self, nc=80, nm=32, npr=256, reg_max=16, end2end=False, ch=()):
        ...
        # [개선됨] nn.ModuleList 대신 단일 Sequential 사용
        # ch[0]는 해상도가 가장 높은 P3 피처맵(원본의 1/8 크기)의 채널 수입니다.
        # Boundary는 고해상도 정보가 중요하므로 x[0]만 사용하여 단일 텐서를 출력합니다.
    ...
    def forward(self, x):
        if self.training:
            # 1. 🚨 원본 x가 오염되기 전에 Boundary(pb) 먼저 계산
            # x[0]만 통과시키므로 출력 shape는 (Batch, 1, H/8, W/8)인 단일 텐서가 됩니다.
            pb = self.cv5(x[0]) 
            
            # 2. 기존 부모 클래스 로직 수행 (이 과정에서 x가 덮어씌워짐)
            p = super().forward(x) 
            ...
            if isinstance(p, dict):
                ...
                # 기존 p는 보통 3개의 텐서 튜플을 반환하므로, 끝에 pb를 추가해 4개짜리 튜플로 만듭니다.
                # 이렇게 하면 Step 4 Loss 함수에서 preds[3]으로 정확히 pb를 꺼낼 수 있습니다.
                return p + (pb,)
            else:
                return p, pb 
        else:
            # 추론(Inference) 시에는 Boundary 계산 자체를 생략 -> 연산량 이득
            return super().forward(x)