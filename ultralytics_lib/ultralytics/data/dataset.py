...
def generate_boundary_and_distance_map(masks_tensor, distance_clip=50):
    """
    Args:
        masks_tensor: 정답 마스크 텐서 (N, H, W) 형태. (0과 1로 이루어짐)
        distance_clip: 거리 맵의 최대 거리 한계값 (이 이상은 가중치를 주지 않음)
    Returns:
        boundaries: 추출된 경계선 마스크 텐서 (N, H, W)
        distance_maps: 경계선 기반 가중치 맵 텐서 (N, H, W)
    """
    # 텐서를 numpy로 변환 (OpenCV 연산을 위해)
    ...
    for i in range(N):
        ...
        # 1. Boundary 추출 (Morphological Gradient 방식: Dilation - Erosion)
        dilated = cv2.dilate(mask, kernel, iterations=1)
        eroded = cv2.erode(mask, kernel, iterations=1)
        boundary = dilated - eroded
        boundaries_np[i] = boundary
        
        # 2. Distance Transform을 위한 배경 설정 (Boundary가 0, 나머지가 1이 되도록 반전)
        inv_boundary = 1 - boundary
        
        # cv2.DIST_L2: 유클리디안 거리, 5: 마스크 크기
        dist_map = cv2.distanceTransform(inv_boundary, cv2.DIST_L2, 5)
        
        # 3. Distance Map 가중치 변환
        # 경계선에 가까울수록 값이 커지도록 역산 (선택적)
        # 공식 예시: 가중치 = 1 + exp(-distance / sigma) 혹은 선형 클리핑
        dist_map = np.clip(dist_map, 0, distance_clip)
        # 경계선 픽셀의 거리는 0입니다. Loss에서 가중치로 쓸 것이므로 
        # 거리가 가까울수록(값이 작을수록) 높은 가중치를 주도록 정규화/반전합니다.
        weight_map = (distance_clip - dist_map) / distance_clip
        
        # 기본 가중치 1을 보장하고, 경계 근처에서 최대 2가 되도록 설정 (하이퍼파라미터 튜닝 필요)
        weight_map = 1.0 + weight_map 
        
        distance_maps_np[i] = weight_map

    # 다시 PyTorch Tensor로 변환하여 반환
...
class YOLODataset(BaseDataset):
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        # ==========================================
        # [Step 3 커스텀 추가] Boundary 및 Distance Map 생성
        # ==========================================
        if "masks" in new_batch and new_batch["masks"] is not None and len(new_batch["masks"]) > 0:
            # new_batch["masks"]는 현재 배치 내 모든 객체의 마스크가 모인 (Total_N, H, W) 텐서입니다.
            boundaries, distance_maps = generate_boundary_and_distance_map(new_batch["masks"])
            
            # Loss 함수에서 바로 꺼내 쓸 수 있도록 배치 딕셔너리에 담아줍니다.
            new_batch["boundaries"] = boundaries
            new_batch["distance_maps"] = distance_maps
        # ==========================================
